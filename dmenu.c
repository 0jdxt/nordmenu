...
static char *embed;
static int bh, mw, mh;
static unsigned int dmw = 500; /* make dmenu this wide */
...
static void drawhighlights(struct item *item, int x, int y, int maxw) {
    ...
    if (!(strlen(item->text) && strlen(text))) return;

    drw_setscheme(
        drw, scheme[item == sel ? SchemeSelHighlight : SchemeNormHighlight]);
        ...
        if (!fstrncmp(&(*highlight), &text[i], 1)) {
            ...
            *highlight = c;
            ...
            drw_text(drw, x + indent - (lrpad / 2), y,
                     MIN(maxw - indent, TEXTW(highlight) - lrpad), bh, 0,
                     highlight, 0);
            highlight[1] = c;
            i++;
        }
...
static void drawmenu(void) {
    ...
    drw_setscheme(drw, scheme[SchemeNorm]);
    drw_text(drw, mw - TEXTW(numbers), 0, TEXTW(numbers), bh, lrpad / 2,
             numbers, 0);
...
void fuzzymatch(void) {
    ...
    if (number_of_matches) {
        ...
        if (!(fuzzymatches = realloc(
                  fuzzymatches, number_of_matches * sizeof(struct item *))))
        ...
        matches = matchend = NULL;
        for (i = 0, it = fuzzymatches[i];
             i < number_of_matches && it && it->text;
             i++, it = fuzzymatches[i]) {
            appenditem(it, &matches, &matchend);
        }
        free(fuzzymatches);
        ...
        switch (ksym) {
            ...
            case XK_Y:
                XConvertSelection(dpy,
                                  (ev->state & ShiftMask) ? clip : XA_PRIMARY,
    ...
    switch (ksym) {
        ...
        case XK_End:
            ...
            if (next) {
                ...
                curr = prev;
                calcoffsets();
                while (next && (curr = curr->right)) calcoffsets();
            }
            sel = matchend;
            break;
        case XK_Escape:
...
static void buttonpress(XEvent *e) {
    ...
    /* right-click: exit */
    if (ev->button == Button3) exit(1);

    if (prompt && *prompt) x += promptw;
    ...
    */
    if (lines > 0) {
        ...
        for (item = curr; item != next; item = item->right) {
            ...
            if (ev->y >= y && ev->y <= (y + h)) {
                puts(item->text);
                if (!(ev->state & ControlMask)) exit(0);
                sel = item;
                if (sel) {
                    sel->out = 1;
                    drawmenu();
...
#ifdef __OpenBSD__
    if (pledge("stdio rpath", NULL) == -1) die("pledge");
#endif
